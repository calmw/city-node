...
contract IntoCityNodeVote is RoleAccess, Initializable {
    // 投票成功
    event VoteSuccessRecord(address node, bytes32 cityId, uint256 toxNum);

    // 投票状态改变
    event VoteStatusChangeRecord(VoteStatus voteStatus);

    // 申请城市候选人记录
    event ApplyCandidateRecord(address user, uint256 applyFee);
    ...
    function adminSetActivityCriteria(
        uint256 cityLevel_,
        uint256 type_,
        uint256 val_
    ) public onlyAdmin {
    ...
    function adminAddCity(
        ...
        if (!inBytes32List(allCityIds, cityId)) {
            // 添加城市
    ...
    function adminSetNodeInfoStatus(
        address node_,
        uint256 status_
    ) public onlyAdmin {
    ...
    function userEditNodeInfo(
        string calldata avatar_,
        string calldata flag_,
        string calldata nodeName_
    ) public {
    ...
    function applyCityCandidate(bytes32 cityId, uint256 controlRate) external {
        ...
        if (!inAddressList(cityCandidate[cityId], msg.sender)) {
            // 添加城市候选人
        ...
        emit ApplyCandidateRecord(msg.sender, applyCityFee);
    }

    // 全球节点申请城市节点
    function globalNodeApplyCityNode(
        bytes32 cityId,
        uint256 controlRate
    ) external {
        require(!isCityExits(cityId), "cityId error");
        require(
            globalNodeCandidateVotes[msg.sender] > 0,
            "You are not global node"
        );
        require(
            voteStatus == VoteStatus.globalVoting,
            "voting is not in progress"
        );
        if (inAddressList(globalNodeCityCandidate[cityId], msg.sender)) {
            // 添加竞选该城市的全球节点地址
        ...
        if (votes > cityNodeGlobalNodeMaxVotes[cityId]) {
            // 更新票数最高者信息
    ...
    // 检测某地址是否存在
    function inBytes32List(
        bytes32[] memory cityIds,
        bytes32 newCityId
    ) private pure returns (bool) {
    ...
    function vote(address candidate, bytes32 cityId, uint256 toxNum) external {
        ...
        require(
            inAddressList(cityCandidate[cityId], candidate),
            "candidate and cityId are mismatching"
        );
        uint256 userBalance = IERC20(TOX).balanceOf(msg.sender);
        ...
        // 添加该城市的投票人
        if (!inAddressList(cityVoteUsers[cityId], msg.sender)) {
            // 添加给该城市投票的人
        ...
        emit VoteRecord(msg.sender, candidate, cityId, toxNum);
    }
    ...
    //    }
    // 获取某个城市参选人列表
    function getCandidateByCityId(
        bytes32 cityId
    ) public view returns (CandidateInfo[] memory) {
        address[] memory candidates = cityCandidate[cityId];
        uint256 candidatesLength = candidates.length;
        CandidateInfo[] memory candidateInfo = new CandidateInfo[](
            candidatesLength
        );
